...
```mermaid
C4Context
    title System Context Diagram for imgproxy

    Person(user, "End User", "Website or app visitor requesting images")
    Person(developer, "Developer", "Configures and deploys imgproxy")

    System(imgproxy, "imgproxy", "Fast image processing server that resizes, converts, and optimizes images on-the-fly")

    System_Ext(cdn, "CDN", "Caches processed images (Cloudflare, Fastly)")
    System_Ext(webapp, "Web Application", "Requests processed images via signed URLs")
    System_Ext(storage, "Storage Services", "S3, GCS, Azure Blob, Swift, HTTP servers")
    System_Ext(monitoring, "Monitoring Services", "Prometheus, DataDog, New Relic, OpenTelemetry")
    System_Ext(errorTracking, "Error Tracking", "Sentry, Bugsnag, Honeybadder, Airbrake")

    Rel(user, cdn, "Requests image", "HTTPS")
    Rel(cdn, imgproxy, "Fetches processed image", "HTTP/HTTPS")
    Rel(webapp, imgproxy, "Generates signed URLs", "HMAC-SHA256")
    Rel(imgproxy, storage, "Fetches source images", "HTTP/S3/GCS/Azure/Swift")
    Rel(imgproxy, monitoring, "Sends metrics", "Various protocols")
    Rel(imgproxy, errorTracking, "Reports errors", "HTTP API")
    Rel(developer, imgproxy, "Configures via environment variables")
...
```mermaid
C4Container
    title Container Diagram for imgproxy

    Person(client, "Client", "Browser, mobile app, or service")

    System_Boundary(imgproxy, "imgproxy") {
        Container(server, "HTTP Server", "Go net/http", "Handles HTTP/2 requests with TLS, timeouts, and connection pooling")
        Container(router, "Router", "Go", "Routes requests, handles middleware chain, manages request lifecycle")
        Container(processor, "Image Processor", "Go + libvips", "Core image processing pipeline with worker pools")
        Container(security, "Security Layer", "Go", "URL signature verification, source validation, auth")
        Container(transport, "Transport Layer", "Go", "Multi-backend image fetching (S3, GCS, Azure, HTTP)")
        Container(config, "Configuration", "Environment Variables", "200+ configuration options loaded at startup")
        Container(metrics, "Metrics Collector", "Go", "Collects and exports metrics to monitoring services")
    }

    System_Ext(storage, "Storage Backends", "S3, GCS, Azure, Swift, HTTP")
    System_Ext(monitoring, "Monitoring", "Prometheus, DataDog, etc.")
    ContainerDb(vips, "libvips", "C Library", "Fast image processing library")

    Rel(client, server, "GET /signature/options/image.jpg", "HTTPS")
    Rel(server, router, "Routes request")
    Rel(router, security, "Validates signature & source")
    Rel(router, processor, "Processes image")
    Rel(processor, transport, "Fetches source image")
    Rel(transport, storage, "Downloads image", "Protocol specific")
    Rel(processor, vips, "Processes via CGo", "Function calls")
    Rel(processor, metrics, "Records metrics")
    Rel(metrics, monitoring, "Exports metrics")
    Rel(config, server, "Configures")
    Rel(config, processor, "Configures")
...
```mermaid
C4Component
    title Component Diagram - Image Processing Pipeline

    Container_Boundary(processor, "Image Processor") {
        Component(handler, "Processing Handler", "Go", "Orchestrates request processing, manages worker pool and semaphores")
        Component(options, "Options Parser", "Go", "Parses URL paths, decodes base64/plain URLs, extracts processing options")
        Component(imagedata, "Image Data Manager", "Go", "Downloads, validates, and manages image data lifecycle")
        Component(pipeline, "Processing Pipeline", "Go", "15-step sequential image transformation pipeline")
        Component(vipsWrapper, "VIPS Wrapper", "Go/CGo", "Wraps libvips C API with Go-friendly interface")

        Component(prepare, "Prepare", "Go", "Calculate dimensions, load metadata")
        Component(crop, "Crop", "Go", "Apply cropping with gravity")
        Component(scale, "Scale", "Go", "Resize image with various algorithms")
        Component(rotate, "Rotate & Flip", "Go", "Apply transformations")
        Component(filters, "Filters", "Go", "Blur, sharpen, pixelate")
        Component(watermark, "Watermark", "Go", "Apply watermark overlay")
        Component(finalize, "Finalize", "Go", "Convert colorspace, strip metadata")
    }

    Component_Ext(signature, "Signature Validator", "Verifies HMAC-SHA256 signatures")
    Component_Ext(transport, "Transport Layer", "Multi-backend fetching")
    ComponentDb(libvips, "libvips", "C library for image processing")

    Rel(handler, options, "Parses processing options")
    Rel(handler, signature, "Validates signature")
    Rel(handler, imagedata, "Fetches source image")
    Rel(imagedata, transport, "Downloads via backend")
    Rel(handler, pipeline, "Executes pipeline")
    Rel(pipeline, prepare, "Step 1-2")
    Rel(prepare, crop, "Step 5")
    Rel(crop, scale, "Step 6")
    Rel(scale, rotate, "Step 7")
    Rel(rotate, filters, "Step 9")
    Rel(filters, watermark, "Step 15")
    Rel(watermark, finalize, "Final steps")
    Rel(pipeline, vipsWrapper, "All steps use")
    Rel(vipsWrapper, libvips, "CGo calls")
